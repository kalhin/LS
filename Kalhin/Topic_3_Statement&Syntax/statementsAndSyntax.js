const topic_3 = {
    ...
    exercise_3: function(arr) {
        console.log(arr);
        const newArr = [];

        function rundomeArray() {
            const randomNumber = Math.floor(Math.random() * arr.length);
            for (let i = 0; i < randomNumber; i++) {
                newArr.push(arr[Math.floor(Math.random() * arr.length)]); //generate random number in every iterations
            }
            if (newArr.length === 0) {
                return rundomeArray();
            }
    ...
    exercise_9: function(str) {
        console.log(str);
        const array = str.split(', ');
        const numberArray = [];
        const stringArray = [];
        array.forEach(function(element) {
            return isNaN(parseInt(element)) ? stringArray.push(element) : numberArray.push(parseInt(element))
        });
...
// topic_3.exercise_3([12, 34, 23, 56]);
...
topic_3.exercise_9('Red, Green, Blue, White, 1, 3, 4, 5, 7');
...
// topic_3.exercise_26([3, 5, 9, 7, 2, 9, 3, 2]);
