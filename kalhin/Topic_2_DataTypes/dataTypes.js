const topic_2 = {
    ...
    exersice_7: function() {
        const userData = prompt('Please, input your data. Ex:/user/bin/index.js');
        // const regex = new RegExp();
        if (userData.trim().length === 0 || userData === undefined) {
            alert(`Your data ${userData} is not correct`)
        } else {
            const pathnameAndExtention = userData.split('.');
            const baseName = pathnameAndExtention[0].split('/')
            alert(`Your data:
            Filename: ${userData}
            Base name: ${baseName[baseName.length-1]}
            Extention: ${pathnameAndExtention[1]}
            Pathname: ${pathnameAndExtention[0]}`)
        }
    },
